...
- **`BartrApp.jsx`** — Three-tab app (Discover, Matches, Profile). Receives `profile` and `session` props from `main.jsx`. `session.access_token` used directly for all auth headers. Contains `transformProfile(row)`, `enrichWithTradeRequests(profiles)`, `Avatar` component, `useWindowWidth` hook, and `SwipeCard`/`MatchCard` sub-components. Swipe threshold: 100px desktop, 80px mobile. Match detection uses two separate queries. **Second chance mode**: when Discover queue empties, "Give another look →" button re-queues left-swiped profiles; swiping right in this mode PATCHes the existing swipe record. Profile tab click navigates to `/profile/:id` instead of rendering inline. Match signal highlights (banner + skill chips) use green (`#5a9e6f`), not terracotta. **Filter system**: `DEFAULT_FILTERS` object, `isFiltersActive()`, and `applyFiltersToProfiles()` handle 5 filter dimensions: gender preference, age range (dual-handle slider), borough (resolved via `getBorough()`), swap preference, skill category. Filters stored as `jsonb` in `profiles.filters` and PATCHed to DB on apply. Active filter tags shown as dismissible chips below the header. `allProfilesRef` holds the unfiltered pool; `sessionSwipedIds` prevents re-surfacing cards after filter changes. **Pronoun display**: `PRONOUN_MAP` maps `gender` → display pronoun (Woman→She/her, Man→He/him, Non-binary→They/them; Prefer not to say and null omitted). Pronouns shown in SwipeCard location line. **Secondary offering**: `transformProfile` includes `offeringSecondary` and `offeringSecondaryIcon`; SwipeCard shows primary (larger) and secondary (smaller) with `PRIMARY`/`SECONDARY` mini labels when a secondary exists; match signal banner and skill chip highlights check both primary and secondary offerings.
...
- **`EditProfile.jsx`** — Standalone page with sticky header. Photo upload at top (circular avatar + 📷 button). Fields: name, neighborhood, bio, age, gender, instagram, linkedin, availability, swap preference, primary offering, secondary offering (optional), seeking. Required fields (name, neighborhood, age, gender, offering, seeking, swap preference) are marked with a terracotta `*` next to their label — visual only, save is not blocked. Also contains a Trade Request section: shows active request as a preview card (with Edit/Remove), a dashed "+" button to create one, or an inline form with two SkillPickers + qty/unit inputs + note field. Saves photo to Supabase Storage, then PATCHes profile row. Full page reload on save (`window.location.href = "/app"`).
- **`ProfileView.jsx`** — Full profile page at `/profile/:userId`. Self-detects via `session.user.id === userId` (isSelf). Parallel-fetches profile + open trade request via `Promise.all`. Shows: hero photo/initials with gradient overlay, name + age, location + join date (with inline pronoun if gender is set — Woman→She/her, Man→He/him, Non-binary→They/them), Instagram/LinkedIn links, offering block (primary larger + secondary smaller with `PRIMARY`/`SECONDARY` labels when secondary exists), trade request card (with Respond button hidden for self), wants-to-learn grid, availability, swap preference. Sticky bottom CTA: self → Edit Profile + Sign Out + Delete Account link; other → 📌 + Message button. **Delete account**: clicking "Delete Account" shows a confirmation modal ("Are you sure...?" with No/Yes). On confirm, calls the `delete-account` Edge Function which deletes all data server-side, then signs out and redirects to `/`.
