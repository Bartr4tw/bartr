...
Works like a dating app for skills — users swipe through profiles of people offering skills or hobbies. When two people want what the other has, it's a match. Bartr facilitates introductions — what happens after the connection is made is between the individuals involved. Currently open to anyone (no invite code required), NYC-focused.
...
- **`BartrApp.jsx`** — Four-tab app (Browse, Discover, Matches, Profile) at indices 0-3. Browse is the default tab. Receives `profile` and `session` props from `main.jsx`. `session.access_token` used directly for all auth headers. Contains `transformProfile(row)`, `enrichWithTradeRequests(profiles)`, `Avatar` component, `useWindowWidth` hook, `avatarBg(id)` color helper, and `SwipeCard`/`MatchCard` sub-components. **Browse tab**: `BROWSE_CATEGORIES` (CATEGORIES minus "All"), `CATEGORY_EMOJI` map, `browseCounts` (people per category), `browseSkillCounts` (people per skill) all computed from a single profile fetch on first Browse visit. Two separate queries (primary + secondary offering) merged and deduped for the people list. Browse fetches use `getAuthHeaders()` — profiles RLS requires authentication even for SELECT. Skill chips show count inline e.g. "🎸 Guitar (3)". People list rows show avatar (color-consistent via `avatarBg`), name/age, location, swap preference tags, and a green "Wants: X" tag if their seeking includes either of your offerings. **Discover tab** (index 1, icon 🔀): swipe card interface. Swipe right button icon is 🤝. Swipe threshold: 100px desktop, 80px mobile. **Second chance mode**: when queue empties, "Give another look →" re-queues left-swiped profiles; swiping right PATCHes the existing swipe record. **Matches tab** (index 2, icon 🤝). Profile tab (index 3) navigates to `/profile/:id`. Match signal highlights (banner + skill chips) use green (`#5a9e6f`). **Filter system**: `DEFAULT_FILTERS`, `isFiltersActive()`, `applyFiltersToProfiles()` handle 5 dimensions: gender, age range, borough, swap preference, skill category. `allProfilesRef` holds the unfiltered pool; `sessionSwipedIds` prevents re-surfacing after filter changes. **Pronoun display**: `PRONOUN_MAP` (Woman→She/her, Man→He/him, Non-binary→They/them). **Secondary offering**: `transformProfile` includes `offeringSecondary`/`offeringSecondaryIcon`; SwipeCard and match signals check both.
- **`Auth.jsx`** — Email + password auth. Three modes: `login`, `signup`, `forgot`. No invite code required. Successful login redirects to `/app` via `window.location.href`. Forgot password calls `supabase.auth.resetPasswordForEmail` with `redirectTo: /reset-password`. Signup page shows "By signing up you agree to our terms of service and privacy policy" with clickable links that open inline modals (`TERMS` and `PRIVACY` constants at top of file). Contact email: bartropen4biz@gmail.com. Governing law: New York.
...
- **`ProfileView.jsx`** — Full profile page at `/profile/:userId`. Self-detects via `session.user.id === userId` (isSelf). Fetches profile + trade request + existing swipe (for other users) via `Promise.all` on load. Shows: hero photo/initials with gradient overlay, name + age, location + join date (with inline pronoun), Instagram/LinkedIn links, offering block (primary larger + secondary smaller with `PRIMARY`/`SECONDARY` labels), trade request card, wants-to-learn grid, availability, swap preference. Sticky bottom CTA: self → Edit Profile + Sign Out + Delete Account link; other → swipe-aware CTA: no swipe → Pass + Message + Like 🤝; liked → "⚡ Liked" + Message; passed → "Passed" + Undo. Like/Undo check for mutual match and show a match overlay on hit. **Delete account**: "Delete Account" → confirmation modal (No/Yes) → calls `delete-account` Edge Function → sign out → redirect to `/`.
- **`Chat.jsx`** — Full-screen messaging at `/chat/:userId`. Supports `?prefillMessage=` query param to pre-fill the input (used by trade request Respond flow). Shows other user's avatar photo if available. Message fetch uses `sender_id=in.(X,Y)&receiver_id=in.(X,Y)`. Supabase Realtime subscription for live messages. Optimistic UI. Back button uses `navigate("/app")`. Header tappable — navigates to `/profile/:userId`. **Does not create match records** — matches are strictly created only on mutual right swipes (in `BartrApp` handleSwipe and `ProfileView` handleLike).
...
- **`skillsData.js`** (`src/lib/`) — Exports `SKILLS` array (34 built-in skills with `{ icon, label, category }`), `CATEGORIES` array (`["All", "Sports & Fitness", "Music", "Tech", "Arts & Crafts", "Food", "Languages", "Other"]`), `NEIGHBORHOODS` object (NYC boroughs → neighborhoods), and `getBorough(neighborhood)` utility. `getBorough` iterates `NEIGHBORHOODS` entries and returns the borough key (e.g. `"Brooklyn"`) for a given neighborhood string, or `null` if not found — used by the borough filter in `BartrApp.jsx`. Staten Island excluded intentionally.
