...
- **`BartrApp.jsx`** — Four-tab app (Browse, Discover, Matches, Profile) at indices 0-3. Browse is the default tab. Reads `location.state?.tab` on mount to open a specific tab (e.g. Chat back button returns to Matches). Contains `transformProfile(row)`, `enrichWithTradeRequests(profiles)`, `Avatar`, `useWindowWidth`, `avatarBg(id)`, `SwipeCard`, `MatchCard`. **Browse tab**: `BROWSE_CATEGORIES`, `CATEGORY_EMOJI`, `browseCounts`, `browseSkillCounts` computed from a single auth-headers profile fetch on first Browse visit. Category grid is a 2-row scrollable container (hidden scrollbar). Skill chips scroll horizontally (no wrap), sorted by popularity descending, count shown inline. Tapping a category auto-selects the most popular skill in it. People list rows show avatar, name/age, location, swap preference tags, solid green "Wants: X" tag. Browse fetches use `getAuthHeaders()`. **Discover tab** (index 1, icon 🔀): left swipe / ✕ = pass (records swipe left); right swipe / 🤝 = opens connection request modal. Queue excludes swiped, matched, and any profile with a pending request in either direction. **Second chance mode** still works for left-swiped profiles. **Matches tab** (index 2, icon 🤝): "REQUESTS" inbox at top shows pending incoming `connection_requests` with Accept/Decline; Accept creates match row + navigates to chat, Decline sets status to declined silently. Match cards tap to chat, avatar taps to profile. Unread indicator (bold name, terracotta dot/border) shown when last message sender is the other person; clears after visiting the chat (`readConversations` ref seeded from `location.state.visited`). **Connection request modal**: 150-char textarea with counter, rate-limit check (5/day), mutual-request auto-match logic. **Filter system**: `DEFAULT_FILTERS`, `isFiltersActive()`, `applyFiltersToProfiles()` — 5 dimensions. **Secondary offering**: `transformProfile` includes `offeringSecondary`/`offeringSecondaryIcon`.
